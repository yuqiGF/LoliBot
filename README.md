...
| AI 聊天 | `@琪琪 你好` | 所有群均支持主动 @；自动接话当前默认关闭 |
...
| 日语入门 | `moji 食べる` | 生成含假名、核心义、简单例句和常用变形的学习卡；`moji 吃饭` 可由中文查日语辞书形，单发 `moji` 会在 N1～N5 五个等级间等概率选级，再从该级全部词条中随机抽取，并在卡片上标出等级 |
...
- 主动 @ 在所有群可用；为控制模型费用，自动接话当前通过配置总开关关闭。
- 会话键由“群号 + 用户号”组成，不同用户、不同群之间不会串记忆。
- 主人身份仅由可信的本地配置判断，不相信昵称、自称或提示词诱导。
- 重新启用自动接话时，仍会过滤重复消息、低信息刷屏以及没有真实讨论来源的 KFC 类高频梗。
